...
public abstract sealed class RequestErrorException extends Exception permits
  ...
  public static @NonNull String clientRequestErrorBaseLabel(@NonNull final String error) {
    return "com.github.emw7.platform.i18n.error.request.client." + error;
  }
...
//  }
  ...
  private static @Nullable Error errorWithMessage(@Nullable final Error error) {
    if (error == null) {
      ...
      return new Error(TranslatorContainer.getTranslator()
          .translate((Locale) null, error.label(), error.params()), error.label(), error.params());
    }
  }
   ...
   * A request error.
   * <p>
   * {@code error} should be obtained with something like this:
   * {@code translate(<language>,label,params)}.
   * <pre>
   ...
   * @param params  the params to be used actual values in the label placeholders; if {@code null}
   *                then it is forced to an unmodifiable empty map
   */
  ...
  public record Error(@Nullable String message, @NonNull String label,
    ...
    //  if specified value is null; can be verified with he following test:
  ...
  protected RequestErrorException(@Nullable final Throwable cause, @NonNull final String type,
    ...
    this.traceId = LogTracingUtil.traceId(TracerContainer.getTracer());
    this.spanId = LogTracingUtil.spanId(TracerContainer.getTracer());
  }
