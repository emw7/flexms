...
public final class BooleanMapper {
  ...
  /**
   * Returns {@code true} when specified string is any got these (case-insensitive): true, 1, t,
   * on, yes, y; {@code false} otherwise
   * <p>
   * Returns {@code null} if {@code b} is {@code null}.
   *
   * @param b the string to map to {@code boolean}
   * @return {@code true} when specified string is any of these (case-insensitive): true, 1, t,
   * on, yes, y; {@code false} otherwise
   */
