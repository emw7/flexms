...
//    final String id=  UUID.randomUUID().toString();
    ...
    throw new UnknownDependencyErrorServerException(
        new UnknownDependencyErrorException(new Object(), "bar", "q", "v1", "ahia", null) {
        }, new Id("00000"));
    ...
    for (int i = 0; i < num; i++) {
      ...
      final Bar bar = new Bar(uuid.toString(), i + "-bar", i + ".bar", Severity.valueOf(
          ThreadLocalRandom.current()
              .nextInt(Severity.NONE.severity(), Severity.CRITICAL.severity() + 1)),
          State.valueOf(
